<template>
    <div id='root'>
        ...
        <div class='about' v-if='page == -1'>
            ...
            <p>By <a href='https://github.com/just-it-O/xiaoxiaoxin'>SUPERTSY5</a></p>
...
export default {
    ...
    data() {
        return {
            ...
            items: [
                ...
                {
                    name: "开始时间",
                    value: "2025-04-24 12：00"
                },
                {
                    name: "结束时间",
                    value: "2025-04-24 21：00"
                },
                {
                    name: "时长（天）",
                    value: "0.5"
    ...
    methods: {
        go() {
            window.localStorage.setItem('last-data', JSON.stringify({
            ...
            // 使用window.open打开新页面，可根据需要改为window.location.href进行页面跳转
            window.open('yiban前端1.html', '_self'); 
        },
        setButtonCancel(el) {
            if (el) {
        ...
        handleImageUpload(event) {
            ...
            if (this.evidenceImages.length + files.length > 5) {
                alert('最多只能上传5张图片');
                return;
            }
            for (let i = 0; i < files.length; i++) {
                ...
                reader.onload = (e) => {
                    this.evidenceImages.push(e.target.result);
                };
